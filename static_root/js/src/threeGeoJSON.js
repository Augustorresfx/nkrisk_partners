...
export function drawThreeGeo({ json, radius, materalOptions }) {
  ...
  function drawFilledPolygon(x_values, y_values, z_values, materialOptions) {
    const shape = new THREE.Shape();
  
    for (let i = 0; i < x_values.length; i++) {
      if (i === 0) {
        shape.moveTo(x_values[i], y_values[i]);
      } else {
  ...
  function drawLine(x_values, y_values, z_values, options) {
    ...
    // Fija un color específico
    const color = new THREE.Color(0x0170BA,); 
